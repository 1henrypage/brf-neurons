...
@jax.custom_vjp
def StepGaussianGrad(x: jnp.ndarray):
    return step(x)

def StepGaussianGrad_fwd(x: jnp.ndarray):
    y = step(x)
    return y, x

def StepGaussianGrad_bwd(x: jnp.ndarray, g: jnp.ndarray):
    dfdx = gaussian(x)
    return (g * dfdx,)
...
# --- 2. StepLinearGrad ---
@jax.custom_vjp
def StepLinearGrad(x: jnp.ndarray):
    return step(x)

def StepLinearGrad_fwd(x: jnp.ndarray):
    y = step(x)
    return y, x

def StepLinearGrad_bwd(x: jnp.ndarray, g: jnp.ndarray):
...
@jax.custom_vjp
def StepExpGrad(x: jnp.ndarray):
    return step(x)

def StepExpGrad_fwd(x: jnp.ndarray):
    y = step(x)
    return y, x

def StepExpGrad_bwd(x: jnp.ndarray, g: jnp.ndarray):
...
@jax.custom_vjp
def StepDoubleGaussianGrad(x: jnp.ndarray):
    return step(x)

def StepDoubleGaussianGrad_fwd(x: jnp.ndarray):
    y = step(x)
    return y, x

def StepDoubleGaussianGrad_bwd(x: jnp.ndarray, g: jnp.ndarray):
    p = 0.15
    scale = 6.0
    length = 0.5
    gamma = 0.5
...
@jax.custom_vjp
def StepMultiGaussianGrad(x: jnp.ndarray):
    return step(x)

def StepMultiGaussianGrad_fwd(x: jnp.ndarray):
    y = step(x)
    return y, x

def StepMultiGaussianGrad_bwd(x: jnp.ndarray, g: jnp.ndarray):
...
@jax.jit
def FGI_DGaussian(x: jnp.ndarray):
